...
The pretrained GloVe file is not stored in this repository because of its size. Obtain the relativized embedding file from the course materials and place it in `data/` with this exact name:
...
Run all commands from the `Homework1` directory, where `sentiment_classifier.py` is located. If you are at the repository root, enter the directory first:
...
- Run the program from `Homework1` so the default relative data paths resolve correctly.
...
- Generated predictions, Python caches, virtual environments, IDE settings, model checkpoints, and GloVe files are excluded through the repository's root `.gitignore`.
